...
test("save data round-trips stable placement fields", () => {
  const inventory = placeItem(createInventory(3), catalog[0], 1, 1, true, "knife-1");
...
test("imports reject unknown properties and prototype pollution keys", () => {
  ...
  const placement = { instanceId: "knife-1", itemId: "knife", row: 0, column: 0, rotated: false, flipped: false, shapeIndex: 0, html: "<script>" };
...
test("imports reject unknown items, duplicate IDs, overlap, and invalid bounds", () => {
  ...
  const placement = { instanceId: "one", itemId: "knife", row: 0, column: 0, rotated: false, flipped: false, shapeIndex: 0 };
...
test("imports enforce Strength, file size, and primitive field types", () => {
  ...
  assert.throws(() => parseInventorySave(JSON.stringify({ ...base, placements: [{ instanceId: "one", itemId: "knife", row: "0", column: 0, rotated: false, flipped: false, shapeIndex: 0 }] }), catalog), /coordinates/);
  assert.throws(() => parseInventorySave(JSON.stringify({ ...base, placements: [{ instanceId: "one", itemId: "knife", row: 0, column: 0, rotated: false, flipped: false, shapeIndex: 99 }] }), catalog), /transform/);
});
